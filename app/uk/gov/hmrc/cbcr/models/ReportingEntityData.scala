...
object ReportingEntityData{
  ...
  implicit val reads:Reads[ReportingEntityData]= (
    ...
    (JsPath \ "additionalInfoDRI").read[List[DocRefId]].orElse((JsPath \ "additionalInfoDRI").readNullable[DocRefId].map(docRefId => if(docRefId.isDefined) List(docRefId.get) else List.empty[DocRefId])) and
