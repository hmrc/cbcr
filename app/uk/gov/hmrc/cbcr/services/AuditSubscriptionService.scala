...
class AuditSubscriptionService @Inject() (repo:SubscriptionDataRepository,
  ...
  if (auditSubscriptions) {
    ...
    repo.getSubscriptions(subscriptions).map(sd => sd.foreach( s => auditSubscriptionDetails(s).map {
      case AuditResult.Success => Logger.info(s"Successfully audited SubscriptionDetails of CBCId ${s.cbcId.toString}")
      case AuditResult.Failure(msg, _) => Logger.warn(s"Unable to audit SubscriptionDetails of CBCId ${s.cbcId.toString} $msg")
      case AuditResult.Disabled => Logger.warn(s"Auditing is disabled for SubscriptionDetails of CBCId ${s.cbcId.toString}")
    }))
