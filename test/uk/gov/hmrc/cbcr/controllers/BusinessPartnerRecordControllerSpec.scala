...
class BusinessPartnerRecordControllerSpec extends UnitSpec with MockAuth {
  ...
  "The BusinessPartnerRecordController" should {
    "respond with a 200 if the UTR is matched" in {
      ...
      when(dc.lookup(EQ(utr))(using any())).thenReturn(Future.successful(HttpResponse(Status.OK, response, headers)))
    ...
    "respond with a 404 if the UTR is not found" in {
      val utr = "700000002"
      val fakeRequestSubscribe = FakeRequest("GET", "/getBusinessPartnerRecord")
      when(dc.lookup(EQ(utr))(using any())).thenReturn(Future.successful(HttpResponse(Status.NOT_FOUND, "404")))
    ...
    "respond with a 500 if the DES service is unavailable" in {
      val utr = "700000002"
      val fakeRequestSubscribe = FakeRequest("GET", "/getBusinessPartnerRecord")
      when(dc.lookup(EQ(utr))(using any()))
        .thenReturn(Future.successful(HttpResponse(Status.INTERNAL_SERVER_ERROR, "500")))
    ...
    "respond with a 400 if the DES service returns BAD_REQUEST" in {
      val utr = "700000002"
      val fakeRequestSubscribe = FakeRequest("GET", "/getBusinessPartnerRecord")
      when(dc.lookup(EQ(utr))(using any())).thenReturn(Future.successful(HttpResponse(Status.BAD_REQUEST, "400")))
