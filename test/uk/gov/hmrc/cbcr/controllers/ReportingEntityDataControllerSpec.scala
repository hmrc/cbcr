...
class ReportingEntityDataControllerSpec extends UnitSpec with MockitoSugar with ScalaFutures with MockAuth{
  ...
  val red = ReportingEntityData(NonEmptyList(docRefId,Nil),Some(docRefId),docRefId,TIN("90000000001","GB"),UltimateParentEntity("Foo Corp"),CBC701,Some(LocalDate.now()),Some(LocalDate.now()))

  val pred = PartialReportingEntityData(List(DocRefIdPair(docRefId,None)),Some(DocRefIdPair(docRefId,None)),DocRefIdPair(docRefId,None),TIN("90000000001","GB"),UltimateParentEntity("Foo Corp"),CBC701,Some(LocalDate.now()),Some(LocalDate.now()))
